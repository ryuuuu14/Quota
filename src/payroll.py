...
from database import get_connection, compute_total_12m_salary, compute_pro_rata_salary, get_base_salary
...
def run_payroll_cycle(timeframe_id, teacher_ids=None):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) as cnt FROM activity_logs WHERE timeframe_id = ?", (timeframe_id,))
        result["activity_count"] = cursor.fetchone()['cnt']

        # Check how many GUEST teachers exist
        guest_count_query = "SELECT COUNT(*) FROM teachers WHERE employment_type = 'GUEST'"
        guest_params_count = []
        if teacher_ids:
        ...
        if teacher_ids:
            placeholders = ",".join("?" for _ in teacher_ids)
            cursor.execute(f"DELETE FROM payroll_records WHERE timeframe_id = ? AND teacher_id IN ({placeholders})", (timeframe_id, *teacher_ids))
        else:
        ...
        for log in guest_logs:
            ...
            if "LV ThS" in act_name or "thạc sĩ" in act_name.lower():
                ...
                amount = calculate_thesis_pay(log['guest_rank'], 'MASTER', log['total_qty'])
                qty_to_log = log['total_qty']
            elif "LA TS" in act_name or "tiến sĩ" in act_name.lower():
                task_type = 'Phản biện độc lập luận án tiến sĩ' if 'Phản biện' in act_name else 'Đánh giá luận án tiến sĩ'
                amount = calculate_thesis_pay(log['guest_rank'], 'PHD', log['total_qty'])
                qty_to_log = log['total_qty']
            ...
            if amount > 0:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (log['teacher_id'], timeframe_id, task_type, qty_to_log, amount, today_str))
        ...
        if df_metrics is not None and not df_metrics.empty:
            for _, row in df_metrics.iterrows():
                ...
                base_amount, base_hours = calculate_base_pay(total_12m_salary, standard_hours, actual_hours)
                if base_amount > 0:
                    cursor.execute('''
                        INSERT INTO payroll_records (teacher_id, timeframe_id, task_type, quantity, amount_vnd, log_date)
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (t_id, timeframe_id, 'Lương cơ bản theo GC', base_hours, base_amount, today_str))
                    result["base_count"] += 1
                    result["total_vnd"] += base_amount

                # Overtime supplement if actual exceeds standard
                if actual_hours > standard_hours:
                    ot_amount, capped_overtime = calculate_overtime_pay(total_12m_salary, standard_hours, actual_hours)
                    if ot_amount > 0:
                        cursor.execute('''
                            INSERT INTO payroll_records (teacher_id, timeframe_id, task_type, quantity, amount_vnd, log_date)
                            VALUES (?, ?, ?, ?, ?, ?)
                        ''', (t_id, timeframe_id, 'Vượt định mức GC (Thanh toán cuối năm)', capped_overtime, ot_amount, today_str))
        ...
        if result["total_guest_count"] == 0 and result["total_teacher_count"] == 0:
            d.append("Không có cán bộ nào trong hệ thống. Hãy thêm cán bộ tại tab Quản lý Cán bộ.")
        if result["guest_no_activities"] > 0:
            d.append(f"{result['guest_no_activities']} khách mời không có nhật ký hoạt động cho kỳ này.")
        if result["guest_no_rank"] > 0:
            d.append("Một số khách mời chưa có cấp bậc (guest_rank).")
        if result["skipped_no_salary"] > 0:
            d.append(f"{result['skipped_no_salary']} cán bộ/giảng viên chưa có hệ số lương hoặc tổng lương 12 tháng.")
        if result["skipped_zero_norm"] > 0:
            d.append(f"{result['skipped_zero_norm']} cán bộ/giảng viên có định mức GC bằng 0 (có thể do thiếu chức danh/chức vụ).")
        if result["skipped_no_overtime"] > 0:
            d.append(f"{result['skipped_no_overtime']} cán bộ/giảng viên không có giờ vượt GC.")
        if result["teacher_no_metrics"] > 0:
            d.append(f"{result['teacher_no_metrics']} cán bộ có lương nhưng không có trong bảng metrics (có thể do không có nhật ký).")
