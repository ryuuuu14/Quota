...
def authenticate_user(username: str, password: str) -> dict | None:
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT username, password, role, department_name, teacher_id FROM admin_users WHERE username = ?", (username,))
        row = cursor.fetchone()
        if not row:
            return None
        ...
        try:
            ...
            if stored_password == password:
                ...
                cursor.execute("UPDATE admin_users SET password = ? WHERE username = ?", (hashed, username))
                conn.commit()
                return {
                    "username": row["username"],
                    "role": role,
...
def logout():
    ...
    """
    for key in ["is_admin", "admin_username", "user_role", "user_department", "user_teacher_id"]:
...
def get_department_by_code(code):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT name FROM departments WHERE dept_code = ?", (str(code).strip(),))
        row = cursor.fetchone()
        if row:
