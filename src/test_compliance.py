...
def make_log(quantity=1, class_level="Đại học", class_type="Lý thuyết",
             student_count=30, nckh_level=None, is_main_author=True):
    return {
        "quantity": quantity,
...
def make_activity(base_rate=1.0, category="Giảng dạy",
                  is_teaching=True, is_nckh=False):
    return {
...
def run_all():
    ...
    # ≤40 HV → 1.0
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, student_count=40), act_ly_thuyet),
        10.0,
        label="[Đ8.1.a] 10 tiết, ≤40 HV → 10.0 GC"
    )
    # 41–60 HV → 1.2
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, student_count=50), act_ly_thuyet),
        12.0,
        label="[Đ8.1.a] 10 tiết, 50 HV → 12.0 GC"
    )
    # 61–80 HV → 1.4
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, student_count=70), act_ly_thuyet),
        14.0,
        label="[Đ8.1.a] 10 tiết, 70 HV → 14.0 GC"
    )
    # >80 HV → 1.5
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, student_count=90), act_ly_thuyet),
        15.0,
    ...
    # ≤25 HV → 1.0
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_type="Ngoại ngữ/CNTT", student_count=25), act_nn),
        10.0,
        label="[Đ8.1.a-NN] 10 tiết, ≤25 HV → 10.0 GC"
    )
    # 26–40 HV → 1.2
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_type="Ngoại ngữ/CNTT", student_count=35), act_nn),
        12.0,
        label="[Đ8.1.a-NN] 10 tiết, 35 HV → 12.0 GC"
    )
    # 41–60 HV → 1.4
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_type="Ngoại ngữ/CNTT", student_count=50), act_nn),
        14.0,
        label="[Đ8.1.a-NN] 10 tiết, 50 HV → 14.0 GC"
    )
    # >60 HV → 1.5
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_type="Ngoại ngữ/CNTT", student_count=65), act_nn),
        15.0,
    ...
    # ≤40 → 1.0
    assert_approx(
        calculate_activity_hours(make_log(quantity=5, class_type="Thực hành", student_count=40), act_th),
        5.0,
        label="[Đ8.1.a-TH] 5 tiết, ≤40 HV → 5.0 GC"
    )
    # 41–55 → 1.2
    assert_approx(
        calculate_activity_hours(make_log(quantity=5, class_type="Thực hành", student_count=48), act_th),
        6.0,
        label="[Đ8.1.a-TH] 5 tiết, 48 HV → 6.0 GC"
    )
    # 56–70 → 1.4
    assert_approx(
        calculate_activity_hours(make_log(quantity=5, class_type="Thực hành", student_count=60), act_th),
        7.0,
        label="[Đ8.1.a-TH] 5 tiết, 60 HV → 7.0 GC"
    )
    # >70 → 1.5
    assert_approx(
        calculate_activity_hours(make_log(quantity=5, class_type="Thực hành", student_count=75), act_th),
        7.5,
    ...
    for class_type in ["Xêmina", "Thảo luận", "Bài tập"]:
        ...
        assert_approx(
            calculate_activity_hours(make_log(quantity=10, class_type=class_type, student_count=50), make_activity()),
            12.0,
    ...
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="Thạc sĩ", student_count=50), act_thac_si),
        13.0,
        label="[Đ8.1.b] 10 tiết Thạc sĩ, ≤50 HV → 13.0 GC"
    )
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="Thạc sĩ", student_count=60), act_thac_si),
        15.0,
    ...
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="Tiến sĩ", student_count=5), make_activity()),
        20.0,
    ...
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="LLCT Trung cấp", student_count=45), act_llct),
        10.0,
        label="[Đ8.1.d] 10 tiết LLCT TC, ≤50 HV → 10.0 GC"
    )
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="LLCT Trung cấp", student_count=55), act_llct),
        12.0,
    ...
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="LLCT Cao cấp", student_count=50), make_activity()),
        13.0,
        label="[Đ8.1.đ] 10 tiết LLCT CC, ≤50 HV → 13.0 GC"
    )
    assert_approx(
        calculate_activity_hours(make_log(quantity=10, class_level="LLCT Cao cấp", student_count=60), make_activity()),
        15.0,
    ...
    act_nckh_quoc_te = make_activity(base_rate=200.0, category="NCKH", is_teaching=False, is_nckh=True)
    assert_approx(
    ...
    act_nckh_thi_dau_cap_bo = make_activity(base_rate=72.0, category="NCKH - Hướng dẫn thi đấu", is_teaching=False, is_nckh=True)
    assert_approx(
    ...
    act_nckh_thi_dau_cap_truong = make_activity(base_rate=16.0, category="NCKH - Hướng dẫn thi đấu", is_teaching=False, is_nckh=True)
    assert_approx(
    ...
    assert_approx(calc_required_gc(270, 40.0), 162.0, label="[Đ7] Trưởng khoa GV TN → 270 × 60% = 162.0 GC/year")

    # Hiệu trưởng (90% reduction) + GS/PGS TN (330 GC base) = 330 * 10% = 33
    assert_approx(calc_required_gc(330, 90.0), 33.0, label="[Đ7] Hiệu trưởng GS/PGS TN → 330 × 10% = 33.0 GC/year")

    # Phó Trưởng khoa (30% reduction) + GVC TN (300 GC base) = 300 * 70% = 210
    assert_approx(calc_required_gc(300, 30.0), 210.0, label="[Đ7] Phó TK GVC TN → 300 × 70% = 210.0 GC/year")

    # Công tác tại phòng không giữ chức vụ (60% reduction) + GV XH (250) = 250 * 40% = 100
    assert_approx(calc_required_gc(250, 60.0), 100.0, label="[Đ7] CT phòng GV XH → 250 × 40% = 100.0 GC/year")
    ...
    def calc_maternity_reduction(base_gc, maternity_weeks, nursing_weeks_under12, total_weeks=44):
        """
    ...
    assert_approx(result_c, 139.4, tolerance=1.0, label="[Đ10.3.c] Phạm Thị C năm 2025-2026 ≈ 139.4 GC giảm")
    ...
    def calc_tro_giang_reduction(base_gc, remaining_weeks, reduction_pct=50.0, total_weeks=44):
    ...
    assert_approx(result_a, 61.4, tolerance=0.5, label="[Đ10.3.a] Nguyễn Văn A năm 2025-2026 → 61.4 GC giảm")
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT name, base_teaching_hours_natural, base_teaching_hours_social, base_nckh_hours FROM titles")
        ...
        for title_name, expected in EXPECTED_NORMS.items():
            if title_name in db_titles:
                ...
                assert_approx(row["base_teaching_hours_natural"], expected["natural"], tolerance=0,
                              label=f"[Đ6] {title_name} TN → {expected['natural']} GC/year")
                assert_approx(row["base_teaching_hours_social"], expected["social"], tolerance=0,
                              label=f"[Đ6] {title_name} XH → {expected['social']} GC/year")
                assert_approx(row["base_nckh_hours"], expected["nckh"], tolerance=0,
    ...
    EXPECTED_ROLES = {
        ...
        "Công tác tại phòng, trung tâm không giữ chức vụ lãnh đạo": {"teaching_reduction_pct": 60.0},
    }
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("SELECT name, teaching_reduction_pct FROM reduction_rules WHERE rule_type = 'ROLE'")
        rows = cursor.fetchall()
        conn.close()
        ...
        for role_name, expected in EXPECTED_ROLES.items():
            if role_name in db_roles:
                assert_approx(db_roles[role_name]["teaching_reduction_pct"],
                              expected["teaching_reduction_pct"], tolerance=0,
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("SELECT name, base_conversion_rate FROM activity_types WHERE category = 'NCKH - Hướng dẫn thi đấu'")
        rows = cursor.fetchall()
        conn.close()
        ...
        for act_name, expected_rate in EXPECTED_COMPETITION_ACTIVITIES:
            if act_name in db_acts:
                assert_approx(db_acts[act_name], expected_rate, tolerance=0,
    ...
    def calc_vidu1():
        ...
        reduced_ptk = (base_gc * (1 - ptk_role_pct/100) * (ptk_weeks/44.0)) * (ptk_event_weeks / ptk_weeks)
        reduced_tk = (base_gc * (1 - tk_role_pct/100) * (tk_weeks/44.0)) * (tk_event_weeks / tk_weeks)
    ...
    def calc_vidu2():
        ...
        red_g1_nuoi_con = (g1_base * (g1_weeks/44.0)) * 0.15 * (g1_nuoi_con / g1_weeks)
        ...
        red_g2_nuoi_con = (g2_base * (g2_weeks/44.0)) * 0.15 * (g2_nuoi_con / g2_weeks)
    
        total_reduced = red_g1_thai_san + red_g1_nuoi_con + red_g2_di_hoc + red_g2_nuoi_con
        return total_reduced

    red_2 = calc_vidu2()
    assert_approx(red_2, 146.4, tolerance=0.1, label="[Ví dụ 2] Bùi Thị X giảm 146.4 GC")
    ...
    # 5 ngày
    assert_approx(calculate_t04_weeks(date(2025, 9, 1), date(2025, 9, 6)), 1.0, tolerance=0, label="5 ngày -> 1.0 tuần")
    # 6 ngày
    ...
    assert_approx(calculate_activity_hours(log_nn, act_ly_thuyet), 15.0, label="10 tiết lý thuyết bằng tiếng NN -> 15.0 GC")
    ...
    def simulate_gs_at_phong(title, rule_name, rule_nckh_pct):
        if rule_name == 'Công tác tại phòng không giữ chức danh (Giảm NCKH)' and title == 'Giáo sư, Phó Giáo sư':
            n_red = 0.0
        else:
            n_red = rule_nckh_pct
        return n_red

    assert_approx(simulate_gs_at_phong('Giáo sư, Phó Giáo sư', 'Công tác tại phòng không giữ chức danh (Giảm NCKH)', 50.0), 0.0, label="GS tại phòng -> 0% giảm NCKH")
    assert_approx(simulate_gs_at_phong('Giảng viên', 'Công tác tại phòng không giữ chức danh (Giảm NCKH)', 50.0), 50.0, label="GV tại phòng -> 50% giảm NCKH")
    ...
    assert_approx(apply_auto_compensation(10, -5)[0], 5.0, label="Bù GC sang NCKH: GC còn 5")
    assert_approx(apply_auto_compensation(10, -5)[1], 0.0, label="Bù GC sang NCKH: NCKH còn 0")
    assert_approx(apply_auto_compensation(-10, 5)[0], -5.0, label="Bù NCKH sang GC: GC còn -5")
    assert_approx(apply_auto_compensation(-10, 5)[1], 0.0, label="Bù NCKH sang GC: NCKH còn 0")
