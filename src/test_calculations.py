...
def test_boi_duong():
    log_row = {"quantity": 10, "class_level": "Bồi dưỡng", "student_count": 100}
    activity_type_row = {
        "category": "Giảng dạy",
        "is_teaching_activity": 1,
...
def test_bui_thi_x():
    ...
    try:
        ...
        teacher_row = c.execute(
            "SELECT id FROM teachers WHERE name = 'Bùi Thị X'"
        ).fetchone()
        tf_row = c.execute(
            "SELECT id, start_date FROM timeframes WHERE name = 'Năm học 2025-2026'"
        ).fetchone()
        ...
        num_holidays = c.execute(
            "SELECT COUNT(*) FROM academic_holidays WHERE timeframe_id = ?",
            (tf_row[0],),
        ).fetchone()[0]
        ...
        assert abs(req_gc - expected_req) < 0.2, (
            f"Expected required GC around {expected_req}, got {req_gc}"
        )
        assert abs(red_gc - expected_red) < 0.2, (
            f"Expected reduced GC around {expected_red}, got {red_gc}"
        )
    finally:
