...
try:
    from calculations import (
        calculate_teacher_metrics,
        get_conversion_limits,
        calculate_department_compensation,
        get_teacher_formula_breakdown,
    )
...
def _render_conversion_suggestions(df_display, selected_tf_id, conn):
    with st.expander("Quy đổi & Bù trừ Giờ chuẩn thủ công", expanded=False):
        st.markdown(
            "<p style='font-size: 14px; color: var(--md-on-surface-variant);'>Gợi ý quy đổi thủ công dựa trên số giờ thừa/thiếu (Điều 12). Bấm nút 'Quy đổi' để áp dụng.</p>",
    ...
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div>
            <span class="material-symbols-outlined" style="color: var(--md-amber); font-size: 32px;">verified</span>
                    ...
                    if col_reset.button(
                        "Hủy quy đổi", key=f"reset_{row['id']}_{selected_tf_id}"
                    ):
                        try:
    ...
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div>
            <span class="material-symbols-outlined" style="color: var(--md-green); font-size: 32px;">sync</span>
                    ...
                    if col_btn.button(
                        f"Quy đổi: {limits['max_nckh_to_spend']:.1f} NCKH → {limits['gc_gained']:.1f} GC",
                        key=f"n2g_{row['id']}_{selected_tf_id}",
                    ):
                        try:
                            with get_connection() as conn_write:
                                cursor_write = conn_write.cursor()
    ...
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div>
            <span class="material-symbols-outlined" style="color: var(--md-green); font-size: 32px;">sync</span>
                    ...
                    col_btn, _ = st.columns([3, 7])
                    if col_btn.button(
                        f"Quy đổi: {limits['max_gc_to_spend']:.1f} GC → {limits['nckh_gained']:.1f} NCKH",
                        key=f"g2n_{row['id']}_{selected_tf_id}",
                    ):
                        try:
                            with get_connection() as conn_write:
                                cursor_write = conn_write.cursor()
                ...
                limits["warning"]
                and row["nckh_vuot_thieu_sau_quy_doi"] < 0
                and row["gc_vuot_thieu_sau_quy_doi"] > 0
            ):
                has_suggestion = True
                warning_text = (
                    limits["warning"].replace("NCKH", "NCKH").replace("Giảng dạy", "GC")
                )
                st.warning(
                    f"**{row['name']}**: Thừa GC, thiếu NCKH nhưng **{warning_text}**"
                )
            elif (
                limits["warning"]
                and row["gc_vuot_thieu_sau_quy_doi"] < 0
                ...
                st.warning(
                    f"**{row['name']}**: Thừa NCKH, thiếu GC nhưng **{warning_text}**"
                )

        if not has_suggestion:
            st.markdown(
                '<p style="color: var(--md-on-surface-variant);">Không có gợi ý quy đổi nào tại thời điểm này. Các cán bộ đã hoàn thành hoặc chưa đủ điều kiện.</p>',
                unsafe_allow_html=True,
            )
...
st.markdown(
    '<p style="color: var(--md-on-surface-variant); font-size: 16px;">Giám sát định mức theo thời gian thực và thực hiện quy đổi giờ theo Điều 12.</p>',
    unsafe_allow_html=True,
)
...
if df_tf.empty:
    render_warning_state(
        "Hệ thống chưa có Năm học. Vui lòng tạo trong Cài đặt Hệ thống."
    )
...
if selected_tf_id:
    ...
    if not df_teachers.empty:
        if df_teachers["applied_reductions"].str.contains("Bị ép định mức").any():
            st.warning(
                "⚠️ **Cơ chế Cap (Ép định mức) đang hoạt động:** Số tuần làm việc thực tế đang vượt quá định mức chuẩn của Năm học. Hệ thống tự động co giãn định mức và miễn giảm để bảo đảm công bằng. Vui lòng kiểm tra lại cấu hình ngày nghỉ Lễ/Tết/Hè trong **Cài đặt Hệ thống** nếu điều này là ngoài ý muốn."
            )
        ...
        df_dept_latest = df_dept_hist.drop_duplicates(
            subset=["teacher_id"], keep="first"
        )

        df_display = pd.merge(
            df_teachers, df_dept_latest, left_on="id", right_on="teacher_id", how="left"
        )
        ...
        if is_head_dept:
            ...
            try:
                with conn:
                    cur = conn.cursor()
                    cur.execute(
                        "SELECT * FROM notifications WHERE target_dept = ? AND target_role = 'head' AND is_read = 0 ORDER BY created_at DESC",
                        (dept_name,),
                    )
                    unread_notifications = cur.fetchall()
                    if unread_notifications:
                        st.info(
                            f"**🔔 Bạn có {len(unread_notifications)} thông báo mới về kết quả phê duyệt dữ liệu.**"
                        )
        ...
        tong_nvk = (
            df_display["nvk_da_thuc_hien"].sum()
            if "nvk_da_thuc_hien" in df_display.columns
            else 0
        )
        ti_le_hoan_thanh = (
            (tong_thuc_hien_gc / tong_dinh_muc_gc * 100) if tong_dinh_muc_gc > 0 else 0
        )
        ...
        with col2:
            render_metric_card(
                "Tổng định mức GC", f"{tong_dinh_muc_gc:,.1f}", icon="contract"
            )
        with col3:
            render_metric_card(
                "Tổng GC đã thực hiện", f"{tong_thuc_hien_gc:,.1f}", icon="task_alt"
            )
        with col4:
            render_metric_card(
                "Tỷ lệ hoàn thành GC", f"{ti_le_hoan_thanh:.1f}%", icon="pie_chart"
            )
        with col5:
            render_metric_card(
                "Nhiệm vụ khác (Giờ hành chính)", f"{tong_nvk:,.1f}", icon="assignment"
            )
        ...
        df_display["hoan_thanh_gd"] = df_display["gc_vuot_thieu_sau_quy_doi"].apply(
            lambda x: "Đạt" if x >= 0 else "Không đạt"
        )
        df_display["hoan_thanh_nckh"] = df_display["nckh_vuot_thieu_sau_quy_doi"].apply(
            lambda x: "Đạt" if x >= 0 else "Không đạt"
        )
        ...
        filter_status = st.selectbox(
            "Lọc theo Trạng thái Chung:", ["Tất cả", "Đạt", "Không đạt"]
        )
        if filter_status != "Tất cả":
            df_display = df_display[df_display["Trạng thái Chung"] == filter_status]

        search_query = st.text_input(
            "Tìm kiếm theo từ khóa (tên, đơn vị, chức danh...):",
        ...
        if (
            "gc_give_to_dept" in df_display.columns
            and comp_mode == "Tập thể (theo Đơn vị)"
        ):
        ...
        selected_col_names = st.multiselect(
            "Chọn cột hiển thị",
            options=list(col_mapping.values()),
        ...
        float_cols = [
            ...
            for col in [
                "Định mức gốc GC",
                "Định mức thực tế GC",
                "Số giờ được miễn giảm",
                "Đã thực hiện (tổng GC)",
                "Trong đó: Kế hoạch khác",
                "Vượt/Thiếu GC",
                "Định mức gốc NCKH",
                "Định mức thực tế NCKH",
                "Đã làm NCKH",
                "Vượt/Thiếu NCKH",
                "Định mức NVK (Giờ hành chính)",
                "Đã làm NVK",
                "Vượt/Thiếu NVK",
                "Nhường cho Đơn vị",
                "Nhận từ Đơn vị",
        ...
        if not _HAS_FORMULA_BREAKDOWN or render_formula_card is None:
            st.info(
                "Tính năng tra cứu công thức chưa sẵn sàng. Vui lòng cập nhật phiên bản mới nhất và khởi động lại ứng dụng."
            )
        else:
            st.markdown(
                '<p style="color: var(--md-on-surface-variant); font-size: 14px; margin-bottom: 16px;">Chọn một nhà giáo để xem chi tiết từng tham số được sử dụng trong tính toán: ngày lịch, ngày nghỉ lễ, ngày làm việc thực tế, số tuần, công thức GC/NCKH và các quy tắc miễn giảm.</p>',
                unsafe_allow_html=True,
            )

            teacher_names = ["— Chọn nhà giáo —"] + sorted(
                df_display["name"].dropna().tolist()
            )
