...
def get_teacher_stats(tf_id):
    ...
    has_salary = cur.execute(
        "SELECT COUNT(*) FROM teachers WHERE employment_type IN ('TEACHER','STAFF') AND (total_12m_salary > 0 OR salary_coefficient > 0)"
    ).fetchone()[0]
    missing_salary = total_ts - has_salary
    guest_count = cur.execute(
...
with c3:
    ...
    render_metric_card(
        "Thiếu lương", stats["missing_salary"], delta=delta, icon="warning"
    )
with c4:
...
if not missing_salary_df.empty:
    with st.expander(
        f"⚠️ {len(missing_salary_df)} cán bộ thiếu tổng lương 12 tháng", expanded=False
    ):
...
with col_run_right:
    n_selected = (
        len(selected_teachers)
        if selected_teachers
        else stats["total_ts"] + stats["guest_count"]
    )
...
selected_ids = (
    [teacher_options[t] for t in selected_teachers] if selected_teachers else None
)
...
with c_run:
    ...
    if st.button(
        ...
        if "error" in result:
            ...
            and result["base_count"] == 0
            and result["overtime_count"] == 0
        ):
            ...
            with st.expander("Chi tiết nguyên nhân & cách khắc phục", expanded=True):
                ...
                for t, d in [
                    (
                        "Nhật ký hoạt động",
                        "Vào **Ghi nhận Hoạt động** → kiểm tra đã có dữ liệu cho kỳ này?",
                    ),
                    (
                        "Tổng lương 12 tháng",
                        "Vào **Quản lý Cán bộ** → Cập nhật → nhập Tổng tiền lương 12 tháng.",
                    ),
                    (
                        "Chức danh/Chức vụ",
                        "Vào **Quản lý Cán bộ** → đảm bảo có Chức danh giảng dạy và Chức vụ (nếu có).",
                    ),
                    (
                        "Định mức GC",
                        "Vào **Dashboard** → nếu Định mức GC=0 → thiếu thông tin chức danh hoặc chức vụ.",
                    ),
                    (
                        "Khách mời",
                        "Vào **Quản lý Cán bộ** → đảm bảo khách mời có Cấp bậc và nhật ký giảng dạy.",
                    ),
                ]:
                    st.markdown(f"**{t}:** {d}")
        else:
            guest_info = (
                f"🎓 {result['guest_count']}" if result["guest_count"] > 0 else "0"
            )
            base_info = (
                f"💰 {result['base_count']}" if result["base_count"] > 0 else "0"
            )
            ot_info = (
                f"⏰ {result['overtime_count']}"
                if result["overtime_count"] > 0
                else "0"
            )
            ...
            with rc4:
                render_metric_card(
                    "Tổng tiền", f"{result['total_vnd']:,.0f}", icon="account_balance"
                )
            skipped_parts = []
            ...
            if result["guest_no_activities"] > 0:
                skipped_parts.append(
                    f"{result['guest_no_activities']} khách mời không có nhật ký"
                )
            if skipped_parts:
...
if df_payroll.empty:
    render_empty_state(
        "Chưa có dữ liệu lương cho kỳ học này. Chạy bảng lương để tạo dữ liệu."
    )
else:
    df_payroll["amount_vnd_str"] = df_payroll["amount_vnd"].apply(
        lambda x: f"{x:,.0f} VNĐ"
    )
        ...
        if st.session_state.get(clear_key, False):
            ...
            with cc1:
                if st.button("Xác nhận xóa", key=f"confirm_{tf_id}", type="primary"):
                    conn = get_connection()
                    conn.execute(
                        "DELETE FROM payroll_records WHERE timeframe_id = ?", (tf_id,)
                    )
                    conn.commit()
                    conn.close()
