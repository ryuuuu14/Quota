...
if df_teachers.empty:
    ...
    teacher_opts = {f"{row['Họ tên']} ({row['Khối môn']})": int(row['id']) for _, row in df_teachers.iterrows()}
    ...
    active_role_names = df_current[df_current['rule_type'] == 'ROLE']['rule_name'].tolist()
    active_event_names = df_current[df_current['rule_type'] == 'SPECIAL']['rule_name'].tolist()
    ...
    # Define the change form helper function
    ...
    # Create Tabs layout
    tab_history, tab_update, tab_exempt = st.tabs([
        "📜 Lịch sử công tác & Miễn giảm",
        "🔄 Thay đổi Chức danh / Đơn vị",
        "➕ Thêm Chức vụ / Sự kiện Miễn giảm"
    ])

    with tab_history:
        # Active exemptions and roles cards
        active_reds_query = """
        ...
        if not df_active_reds.empty:
            st.markdown('<h4 style="margin-top: 12px; margin-bottom: 8px;">💼 Các chức vụ và sự kiện đang diễn ra</h4>', unsafe_allow_html=True)
    ...
    ">
        ...
        <div style="color: var(--md-on-surface-variant); font-size: 0.75rem; margin-top: 2px;">Từ {red['start_date']}</div>
                    ...
                    if st.button("Kết thúc ngay", key=f"end_red_{red['id']}", type="secondary"):
                        cursor = conn.cursor()
        ...
        st.markdown('<h4 style="margin-top: 16px; margin-bottom: 8px;">🕰️ Toàn bộ quá trình công tác</h4>', unsafe_allow_html=True)
        view_mode = st.radio("Lọc lịch sử hiển thị:", ["Tất cả", "Nhân sự (Chức danh/Đơn vị)", "Miễn giảm & Chức vụ"], horizontal=True, key="view_mode_radio")
        ...
        with st.expander("Quản lý lịch sử (Sửa / Xóa lỗi nhập liệu)"):
            ...
            if not df_all_hist.empty:
                ...
                selected_del = st.selectbox("Chọn dòng lịch sử cần xóa", options=list(del_opts.keys()), key="select_del_hist")
                del_id = del_opts[selected_del]

                confirm_key = f"confirm_del_hist_{del_id}"
                if st.session_state.get(confirm_key, False):
                    st.markdown("""
                    <div style="background-color: #fff3cd; color: #856404; padding: 12px; border-radius: 4px; margin-bottom: 8px; font-size: 0.85rem;">
                        ⚠️ Bạn có chắc chắn muốn xóa dòng lịch sử này không? Hành động này sẽ thay đổi định mức tính toán.
                    </div>
                    """, unsafe_allow_html=True)
                    col_del1, col_del2 = st.columns(2)
                    with col_del1:
                        if st.button("🔴 Xác nhận xóa", key=f"btn_confirm_del_hist_{del_id}", type="primary"):
                            cursor = conn.cursor()
                            ...
                            st.rerun()
                    with col_del2:
                        if st.button("Hủy bỏ", key=f"btn_cancel_del_hist_{del_id}"):
                            del st.session_state[confirm_key]
                            st.rerun()
                else:
                    if st.button("Xóa dòng lịch sử đã chọn", key=f"btn_request_del_hist_{del_id}", type="primary"):
                        st.session_state[confirm_key] = True
                        st.rerun()
            else:
                st.info("Không có dữ liệu lịch sử để quản lý.")

    with tab_update:
        action_status = st.radio("Chọn thông tin cần cập nhật", ["Cập nhật Chức danh mới", "Chuyển Đơn vị công tác mới"], horizontal=True, key="action_status_radio")
        if action_status == "Cập nhật Chức danh mới":
            render_change_form("Cập nhật Chức danh mới", "f_title", "name", "SELECT name FROM titles", "TITLE")
        else:
            render_change_form("Chuyển Đơn vị công tác mới", "f_dept", "name", "SELECT name FROM departments", "DEPARTMENT")

    with tab_exempt:
        action_event = st.radio("Chọn loại điều chỉnh", ["Thêm Chức vụ Lãnh đạo mới", "Thêm Sự kiện Miễn giảm (Thai sản, Học tập, ...)"], horizontal=True, key="action_event_radio")
        if action_event == "Thêm Chức vụ Lãnh đạo mới":
            render_change_form("Thêm Chức vụ Lãnh đạo mới", "f_role", "name", "SELECT id, name FROM reduction_rules WHERE rule_type = 'ROLE'", "ROLE")
        else:
            render_change_form("Thêm Sự kiện Miễn giảm mới", "f_event", "name", "SELECT id, name FROM reduction_rules WHERE rule_type = 'SPECIAL'", "SPECIAL")
...
with st.expander("⚠️ Khu vực nguy hiểm: Xóa Hồ sơ Nhà giáo", expanded=False):
    df_all_t = pd.read_sql_query("SELECT id, name, subject_group FROM teachers", conn)
    if not df_all_t.empty:
        del_opts = {f"{row['name']} ({row['subject_group']})": int(row['id']) for _, row in df_all_t.iterrows()}
        ...
        <div style="
            ...
            font-size: 0.9rem;
        ">
            <strong>⚠️ CẢNH BÁO NGUY HIỂM:</strong> Hành động này sẽ xóa vĩnh viễn toàn bộ hồ sơ nhà giáo, lịch sử công tác và tất cả nhật ký hoạt động giảng dạy, nghiên cứu khoa học liên quan. Dữ liệu sau khi xóa sẽ <strong>không thể khôi phục</strong>.
        </div>
        """, unsafe_allow_html=True)
        
        confirm_delete = st.checkbox("Tôi hiểu và xác nhận muốn xóa vĩnh viễn toàn bộ dữ liệu của nhà giáo này.", key="confirm_delete_teacher_check")
