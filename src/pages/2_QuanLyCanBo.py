...
st.markdown(
    '<p style="color: var(--md-on-surface-variant); font-size: 16px;">Quản lý thông tin cơ bản và quá trình công tác của nhà giáo.</p>',
...
if not police_ranks:
    ...
    police_rank_opts = {
        f"{r['rank_group']} — {r['rank_name']} (HS {r['coefficient']})": r
        for r in police_ranks
    }
...
def _render_teacher_profile_card(
    ...
    if highlighted:
        ...
        badge_bg, badge_text, badge_border = badge_themes.get(
            chip_variant, badge_themes["primary"]
        )
...
tab1, tab2, tab3 = st.tabs(
    ["📋 Danh sách & Tìm kiếm", "➕ Thêm mới Hồ sơ", "📥 Nhập dữ liệu từ Excel"]
)
...
with tab1:
    ...
    if df_teachers.empty:
        render_warning_state(
            "Chưa có hồ sơ nhà giáo nào. Vui lòng thêm hồ sơ phía trên."
        )
    else:
        ...
        with col_left:
            ...
            for _, row in df_teachers.iterrows():
                ...
                emp_label = {"TEACHER": "GV", "GUEST": "KM", "STAFF": "CB"}.get(
                    emp, emp
                )
                teacher_list_items.append(
                    (row["name"], row["subject_group"], emp_label, int(row["id"]))
                )
            ...
            st.markdown(
                f'<span style="font-size:12px;color:var(--md-on-surface-variant);">{len(teacher_list_items)} nhà giáo</span>',
                unsafe_allow_html=True,
            )
            ...
            df_current = pd.read_sql_query(
                current_status_query, conn, params=[selected_t_id]
            )
            curr_titles = df_current[df_current["record_type"] == "TITLE"][
                "value_text"
            ].tolist()
            curr_depts = df_current[df_current["record_type"] == "DEPARTMENT"][
                "value_text"
            ].tolist()
            curr_roles = df_current[df_current["record_type"] == "ROLE"][
                "value_text"
            ].tolist()
            if not curr_roles:
                curr_roles = df_current[df_current["rule_name"].notna()][
                    "rule_name"
                ].tolist()
            ...
            emp_type_labels = {
                "TEACHER": "Giảng viên cơ hữu",
                "GUEST": "Giảng viên thỉnh giảng",
            ...
            if t_data["employment_type"] == "GUEST":
                ...
                coeff = (
                    t_data["salary_coefficient"]
                    if pd.notna(t_data["salary_coefficient"])
                    else None
                )
                sal = (
                    t_data["total_12m_salary"]
                    if pd.notna(t_data["total_12m_salary"])
                    else 0
                )
                rank_name = t_data.get("rank_name", "")
                if coeff:
                    rank_line = (
                        f"{rank_name} (HS {coeff})" if rank_name else f"Hệ số {coeff}"
                    )
            ...
            has_any_salary = coeff or (
                pd.notna(t_data["total_12m_salary"]) and t_data["total_12m_salary"] > 0
            )
            if t_data["employment_type"] in ("TEACHER", "STAFF") and not has_any_salary:
                salary_warning = '<div style="margin-top: 8px; padding: 6px 10px; background: #fff3cd; border-radius: var(--radius-sm); font-size: 0.8rem; color: #92400e;">⚠️ Chưa có hệ số lương — không thể tính lương</div>'
            st.markdown(
            ...
            if user is not None:
                with st.expander("✏️ Chỉnh sửa Thông tin cơ bản", expanded=False):
                    ...
                    emp_idx = (
                        emp_types.index(t_data["employment_type"])
                        if t_data["employment_type"] in emp_types
                        else 0
                    )
                    ...
                    new_police_rank_id = (
                        t_data["police_rank_id"]
                        if pd.notna(t_data["police_rank_id"])
                        else None
                    )
                    new_coefficient = (
                        t_data["salary_coefficient"]
                    ...
                    if st.session_state[edit_emp_key] == "GUEST":
                        ...
                        rank_idx = (
                            ranks.index(new_guest_rank)
                            if new_guest_rank in ranks
                            else 0
                        )
                        new_guest_rank = st.selectbox(
                            "Cấp bậc Khách mời", ranks, index=rank_idx
                        )
                    else:
                        ...
                        edit_rank_idx = (
                            rank_keys.index(current_rank_key)
                            if current_rank_key in rank_keys
                            else 0
                        )
                        ...
                        selected_edit_rank = police_rank_opts.get(
                            selected_edit_rank_key
                        )
                    ...
                    with st.form("edit_basic_info_form"):
                        ...
                        new_fem = col_b2.selectbox(
                            ...
                            key=f"edit_gender_{selected_t_id}",
                        )
                            ...
                            if is_admin:
                                ...
                                """,
                                    (
                                        ...
                                        f"Cập nhật thông tin {t_data['name']}",
                                    ),
                                )
        ...
        with col_right:
            if user is not None:
                ...
                if action_type == "Chức danh":
                    ...
                    sql_opts = (
                        "SELECT id, name FROM reduction_rules WHERE rule_type = 'ROLE'"
                    )
                    field_type = "ROLE"
                ...
                if field_type == "RANK":
                    if not is_admin:
                        st.warning(
                            "⚠️ Chỉ Quản trị viên hệ thống mới có quyền thay đổi Cấp bậc hàm (Ngạch lương)."
                        )
                    else:
                        police_ranks = get_police_ranks()
                        if not police_ranks:
                        ...
                        with st.form("dynamic_form_RANK"):
                            ...
                            selected_rank_key = st.selectbox(
                                "Chọn Cấp bậc hàm mới", options=rank_keys
                            )
                            selected_rank = rank_opts.get(
                                selected_rank_key, {"id": None, "coefficient": 0.0}
                            )
                            ...
                            rank_note = c2.text_input(
                                "Ghi chú (nếu có)",
                                placeholder="Ví dụ: Xét nâng bậc lương",
                            )
                    ...
                    if field_type == "SPECIAL" and not is_admin:
                        st.warning(
                            "⚠️ Chỉ Quản trị viên hệ thống mới có quyền thêm Sự kiện miễn giảm."
                        )
                    else:
                        ...
                        if field_type in ["ROLE", "SPECIAL"]:
                            if field_type == "ROLE":
                                unit_type = st.radio(
                                    "Phạm vi công tác",
                                    ["Tại đơn vị giảng dạy", "Phòng/Trung tâm"],
                                    horizontal=True,
                                )
                                school_roles = [
                                    "Hiệu trưởng",
                                ...
                                if unit_type == "Tại đơn vị giảng dạy":
                                    df_opts = df_opts[
                                        df_opts["name"].str.contains(
                                            "Tại đơn vị giảng dạy"
                                        )
                                        | df_opts["name"].isin(
                        ...
                        with st.container(border=True):
                            ...
                            selected_val = st.selectbox(
                                "Chọn giá trị mới", options=options
                            )
                            ...
                            if field_type in ["ROLE", "SPECIAL"]:
                                ...
                                end_date_inner = c2.date_input(
                                    "Ngày kết thúc", disabled=is_ongoing
                                )
                            ...
                            if st.button("Lưu Lịch sử", type="primary"):
                                if not options or options[0] == "(Chưa có dữ liệu)":
                                    ...
                                    active_rec = (
                                        df_current[
                                            df_current["record_type"] == field_type
                                        ]
                                        if field_type in ["TITLE", "DEPARTMENT"]
                                        else df_current[
                                            df_current["rule_type"] == field_type
                                        ]
                                    )

                                    if (
                                        not active_rec.empty
                                        and active_rec.iloc[0]["start_date"]
                                    ):
                                        try:
                                            current_start = datetime.strptime(
                                                active_rec.iloc[0]["start_date"],
                                                "%Y-%m-%d",
                                            ).date()
                                            if start_date_inner <= current_start:
                                                st.error(
                                                    f"Ngày bắt đầu mới phải sau ngày bắt đầu của bản ghi hiện tại ({current_start})!"
                                                )
                                                st.stop()
                                        except Exception:
                                            pass
                                    ...
                                    if is_admin:
                                        if field_type in ["TITLE", "DEPARTMENT"]:
                                            ...
                                            cursor.execute(
                                                "INSERT INTO teacher_role_history (teacher_id, record_type, value_text, start_date) VALUES (?, ?, ?, ?)",
                                                (
                                                    selected_t_id,
                                                    field_type,
                                                    selected_val,
                                            ...
                                            cursor.execute(
                                                "INSERT INTO teacher_role_history (teacher_id, record_type, reduction_rule_id, start_date, end_date, actual_weeks_override) VALUES (?, 'REDUCTION', ?, ?, ?, ?)",
                                                (
                                                    selected_t_id,
                                                    rule_id,
                                                    start_date_inner.isoformat(),
                                        ...
                                        """,
                                            (
                                                c_dept,
                                                f"User {user['username']}",
                                                f"Yêu cầu điều chỉnh {action_type} - {t_data['name']}",
                                            ),
                                        )
                                        batch_id = cursor.lastrowid
            ...
            if not combined.empty:
                ...
                for _, row in combined.iterrows():
                    ...
                    _prefixes = {
                        "Chức danh": "Thay đổi chức danh",
                        "Đơn vị": "Chuyển đơn vị",
                        "Chức vụ": "Bổ nhiệm",
                        "Cấp bậc hàm": "Thay đổi cấp bậc hàm",
                        "Sự kiện": "",
                    }
                    _prefix = _prefixes.get(row["Loại"], "")
                    _detail = (
                        f"{_prefix}: {row['Chi tiết']}"
                        if (_prefix and not _is_first)
                        else row["Chi tiết"]
                    )
                    tl_html += f"""
            ...
            if user is not None:
                with st.expander("⚠️ Xóa dữ liệu lỗi / Xóa Hồ sơ"):
                    ...
                    if not is_admin:
                        st.warning(
                            "⚠️ Chỉ Quản trị viên hệ thống mới có quyền xóa dòng lịch sử."
                        )
                    else:
                        ...
                        """,
                            conn,
                            params=[selected_t_id],
                        )

                        if not df_all_hist.empty:
                            del_opts = {
                                f"[{row['record_type']}] {row['detail']}": row["id"]
                                for _, row in df_all_hist.iterrows()
                            }
                    ...
                    confirm_delete = st.checkbox(
                        "Tôi hiểu và xác nhận xóa vĩnh viễn toàn bộ dữ liệu.",
                        key="confirm_delete_teacher_check",
                    )
                    if st.button(
                        "Xóa vĩnh viễn Nhà giáo này",
                        ...
                        if is_admin:
                            ...
                            """,
                                (
                                    c_dept,
                                    f"User {user['username']}",
                                    f"Yêu cầu xóa hồ sơ - {t_data['name']}",
                                ),
                            )
                            batch_id = cursor.lastrowid
                            ...
                            """,
                                (
                                    ...
                                    1,
                                    "DELETE",
                                    f"Yêu cầu xóa toàn bộ hồ sơ của nhà giáo: {t_data['name']}",
                                    "",
                                    t_data["name"],
                                    t_data["subject_group"],
                                    int(t_data["is_female"]),
    ...
    if user is not None:
        with st.expander("➕ Thêm mới Hồ sơ Nhà giáo", expanded=False):
            ...
            df_roles = pd.read_sql_query(
                "SELECT id, name FROM reduction_rules WHERE rule_type = 'ROLE'", conn
            )
            roles_dict = (
                {row["name"]: row["id"] for _, row in df_roles.iterrows()}
            ...
            create_emp_type = st.selectbox(
                "Loại nhân sự",
                options=list(emp_type_opts.keys()),
                format_func=lambda x: emp_type_opts[x],
                key="create_emp_type",
            )
            ...
            if st.session_state.create_emp_type == "GUEST":
                ...
                create_guest_rank = st.selectbox(
                    "Cấp bậc Khách mời", options=list(GUEST_RATES.keys())
                )
            else:
                selected_rank_key = st.selectbox(
                    "Cấp bậc hàm",
                ...
                selected_rank = police_rank_opts.get(
                    selected_rank_key, {"id": None, "coefficient": 0.0}
                )
            ...
            with st.form("add_teacher_form"):
                ...
                if st.form_submit_button("Lưu Hồ sơ"):
                    if not name.strip():
                        ...
                        initial_title == "(Chưa có dữ liệu)"
                        or initial_dept == "(Chưa có dữ liệu)"
                    ):
                        st.error(
                            "Vui lòng cài đặt Chức danh và Đơn vị trong phần Cài đặt Hệ thống trước."
                        )
                    else:
                        cursor = conn.cursor()
...
with tab3:
    ...
    if is_admin or is_head:
        ...
        depts_list_opts = (
            df_depts_list["name"].tolist() if not df_depts_list.empty else []
        )

        if is_admin:
            dept_name = st.selectbox(
                "Chọn Đơn vị nhập dữ liệu:",
                options=depts_list_opts,
                key="admin_upload_dept",
            )
        ...
        if dept_name:
            ...
            st.info(
                "💡 **Mẹo:** Bạn có thể tải lên bất kỳ bảng Excel nào. Hệ thống hỗ trợ tính năng tự động nhận diện tiêu đề cột thông minh."
            )
            st.markdown("---")
            ...
            if uploaded_teachers is not None:
                if uploaded_teachers.size > 5 * 1024 * 1024:
                    ...
                    if st.session_state.import_step_teachers == 1:
                        ...
                        if st.button(
                            "Tiếp theo →", key="step1_next_teachers_after_upload"
                        ):
                        ...
                        try:
                            ...
                            def _auto_detect_header_row(file_bytes, sheet_name):
                                ...
                                for idx in range(min(21, len(df))):
                                    cells = [
                                        str(c).strip().lower()
                                        for c in df.iloc[idx]
                                        if pd.notna(c)
                                    ]
                                    score = sum(
                                        1 for c in cells if any(k in c for k in known)
                                    )
                                    if score > best_score:
                            ...
                            header_row = _auto_detect_header_row(
                            ...
                            if not headers:
                                ...
                                template_options = ["(Chọn cấu hình đã lưu)"] + list(
                                ...
                                with col_tpl_del:
                                    ...
                                    if selected_tpl != "(Chọn cấu hình đã lưu)":
                                        if st.button(
                                            "🗑️ Xóa cấu hình này", key="delete_tpl_btn"
                                        ):
                                ...
                                def _load_sample_data(
                                    file_bytes, sheet_name, header_row, headers
                                ):
                                    try:
                                        import pandas as pd
                                        ...
                                        if df.empty:
                                            return {}
                                        return {
                                            h: df.iloc[0].get(h, "") for h in headers
                                        }
                                    except Exception:
                                        return {}
                                        ...
                                        for col in req_cols_list:
                                            ...
                                            with rc[2]:
                                                ...
                                                if cur in headers:
                                                    ...
                                                    di = (
                                                        headers.index(m.matched_header)
                                                        + 1
                                                    )
                                                sel = st.selectbox(
                                                    "",
                                    ...
                                    with col_right:
                                        ...
                                        try:
                                            ...
                                            if not df_full.empty:
                                                ...
                                                st.markdown(
                                                    f'<span style="font-size:12px;color:#6b7280;">50 dòng đầu ({n} dòng)</span>',
                                                    unsafe_allow_html=True,
                                                )
                                                st.dataframe(
                                                    head,
                                                ...
                                                st.markdown(
                                                    '<span style="font-size:12px;color:#6b7280;">0 dòng — chỉ có tiêu đề cột</span>',
                                                    unsafe_allow_html=True,
                                                )
                                                st.dataframe(
                                    ...
                                    if missing_required:
                                        ...
                                        try:
                                            ...
                                            if df_parsed.empty:
                                                ...
                                                st.error(
                                                    "File Excel quá lớn (>100.000 dòng). Vui lòng chia nhỏ file."
                                                )
                                            else:
                                                # Validate
                                                errors = validate_teachers_data(
                                                ...
                                                if errors:
                                                    ...
                                                    if (
                                                        st.session_state.import_step_teachers
                                                        == 3
                                                    ):
                                                        if st.button(
                                                            "Tiếp theo →",
                                                            key="step3_next_teachers",
                                                        ):
                                                        ...
                                                        if st.button(
                                                            ...
                                                            try:
                                                                ...
                                                                for (
                                                                    ...
                                                                    role_sdate = normalize_date(
                                                                        row.get(
                                                                            "Ngày bổ nhiệm chức vụ"
                                                                        )
                                                                    )
                                                                    title_sdate = normalize_date(
                                                                        row.get(
                                                                            "Ngày bổ nhiệm chức danh"
                                                                        )
                                                                    )
