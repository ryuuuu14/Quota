...
def test_approval_workflow_teachers(tmp_path):
        ...
        if marker == "NEW":
            ...
            if t_id:
                cursor.execute("""
                    ...
                    WHERE id = ?
                """, (r["subject_group"], r["is_female"], r["employment_type"], r["guest_rank"], r["total_12m_salary"], r["police_rank_id"], r["salary_coefficient"], t_id))
    ...
    cursor.execute("SELECT COUNT(*) as cnt FROM staging_teachers WHERE batch_id = ?", (batch_id,))
...
def test_approval_workflow_activities(tmp_path):
    ...
    if tf_row:
        ...
        cursor.execute("INSERT INTO timeframes (name, start_date, end_date) VALUES ('Năm học 2025-2026', '2025-09-01', '2026-06-30')")
    ...
    if act_row:
        ...
        cursor.execute("INSERT INTO activity_types (name, category, unit, base_conversion_rate) VALUES ('Đọc sách', 'Khác', 'Giờ', 1.0)")
        ...
        if r["diff_marker"] == "NEW":
            ...
            if t_id and tf_id_m and act_id_m:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (t_id, act_id_m, r["log_date"], r["quantity"], r["class_level"], r["class_type"], r["student_count"], r["nckh_level"], r["is_main_author"], r["is_foreign_language_instruction"], r["note"], tf_id_m, conv_hours))
                
    cursor.execute("UPDATE import_batches SET status = 'approved' WHERE id = ?", (batch_id,))
...
def test_approval_workflow_schedule(tmp_path):
    ...
    for r in staging_rows:
        ...
        if t_id and tf_id_m and r["diff_marker"] == "NEW":
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (tf_id_m, t_id, r["subject_name"], r["loai"], r["nhom"], r["si_so"], r["tiet_quy_doi"], r["he_so_tin_chi"], "", r["he_so_lop_dong"], r["tiet_thuc_day"]))
    ...
    cursor.execute("SELECT * FROM bulk_teaching_assignments WHERE teacher_id = ?", (t_a["id"],))
