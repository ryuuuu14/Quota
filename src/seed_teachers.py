...
def ensure_timeframe(conn, cursor):
    ...
    existing = cursor.execute("SELECT id FROM timeframes WHERE name = ?", (TF_NAME,)).fetchone()
    if existing:
        cursor.execute("""
...
def ensure_settings(conn, cursor):
    ...
    for k, v, d in settings:
        try:
            cursor.execute("INSERT INTO settings (key, value, description) VALUES (?,?,?)", (k, v, d))
        except:
            pass
...
def ensure_titles(conn, cursor):
    ...
    for t in titles:
        cursor.execute("INSERT OR REPLACE INTO titles (name, base_teaching_hours_natural, base_teaching_hours_social, base_nckh_hours) VALUES (?,?,?,?)", t)
...
def ensure_departments(conn, cursor):
    ...
    for d in depts:
        try:
            cursor.execute("INSERT INTO departments (name, is_teaching_dept) VALUES (?,?)", d)
        except:
            pass

def get_rule_id(cursor, name):
    row = cursor.execute("SELECT id FROM reduction_rules WHERE name = ?", (name,)).fetchone()
    return row[0] if row else None

def get_act_type_id(cursor, name):
    row = cursor.execute("SELECT id FROM activity_types WHERE name = ?", (name,)).fetchone()
    return row[0] if row else None
...
def seed_teacher_GVBinhThuong(conn, cursor, tf_id):
    ...
    gd_id = get_act_type_id(cursor, 'GD - Lý thuyết ĐH (dùng class_level+student_count để nhân hệ số)')
...
def delete_existing_teachers(conn, cursor):
    ...
    for name in names:
        ...
        if row:
            ...
            cursor.execute("DELETE FROM teacher_role_history WHERE teacher_id = ?", (tid,))
            cursor.execute("DELETE FROM manual_conversions WHERE teacher_id = ?", (tid,))
...
def run():
    ...
    conn.close()
    print("  Done. Teacher records seeded into:", os.environ.get('DB_PATH', 'data/database.sqlite'))
