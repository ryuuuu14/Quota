...
def reviewer_node(state: ComplianceState) -> ComplianceState:
    ...
    if "Điều 8" in rule:
        findings.append({"rule": rule, "status": "FAIL", "issue": "Missing 'Kỹ thuật hình sự' in class_type check."})
    elif "Điều 3.6" in rule or "Điều 12" in rule:
        findings.append({"rule": rule, "status": "FAIL", "issue": "Direct Teaching 50% Constraint Violation. Used tổng GC instead of Giảng dạy trực tiếp."})
    else:
