    ...
    if activity_type_row["category"] == "Giảng dạy":
        ...
        if log_row["class_level"] == "Đại học":
            if log_row["class_type"] in ["Lý thuyết", "Thảo luận", "Bài tập", "Xêmina"]:
                if 41 <= log_row["student_count"] <= 60:
                    multiplier = 1.2
                elif 61 <= log_row["student_count"] <= 80:
                    multiplier = 1.4
                elif log_row["student_count"] > 80:
                    multiplier = 1.5
            elif log_row["class_type"] in ["Ngoại ngữ/CNTT", "Kỹ thuật hình sự"]:
                if 26 <= log_row["student_count"] <= 40:
                    multiplier = 1.2
                elif 41 <= log_row["student_count"] <= 60:
                    multiplier = 1.4
                elif log_row["student_count"] > 60:
                    multiplier = 1.5
            elif log_row["class_type"] == "Thực hành":
                if 41 <= log_row["student_count"] <= 55:
                    multiplier = 1.2
                elif 56 <= log_row["student_count"] <= 70:
                    multiplier = 1.4
                elif log_row["student_count"] > 70:
                    multiplier = 1.5

        # Đào tạo Thạc sĩ
        elif log_row["class_level"] == "Thạc sĩ":
            if log_row["student_count"] <= 50:
                multiplier = 1.3
            else:
                multiplier = 1.5
...
def _generate_timeline_segments(
    tf_start, tf_end, title_recs, dept_recs, role_recs, rules_dict
):
    """
    ...
    for r_list in [title_recs, dept_recs]:
        for _, r in r_list.iterrows():
            ...
            r_end = (
                min(pd.to_datetime(tf_end), pd.to_datetime(r["end_date"]))
                if pd.notnull(r["end_date"])
                else pd.to_datetime(tf_end)
            )
            if r_start <= r_end:
                dates.add(r_start)
...
def _calculate_tro_giang_reductions(
    seg_start,
    seg_end,
    title_name,
    title_recs,
    ...
    if title_name == "Trợ giảng":
        ...
        if p1_start <= p1_end:
            ...
            red_gc = (
                seg_base_gc * (1 - role_t_red / 100.0) * 0.5 * (p1_weeks / std_weeks)
            )
            reduced_gc += red_gc
            reductions_desc.append(
                f"Trợ giảng 12 tháng đầu (giảm 50% x {p1_weeks:.1f} tuần)"
            )
        ...
        if p2_start <= p2_end:
            ...
            red_gc = (
                seg_base_gc * (1 - role_t_red / 100.0) * 0.2 * (p2_weeks / std_weeks)
            )
            reduced_gc += red_gc
            reductions_desc.append(
                f"Trợ giảng tháng 13-24 (giảm 20% x {p2_weeks:.1f} tuần)"
            )

    return reduced_gc, reductions_desc


def _calculate_point2_reductions(
    ...
    for r, rule in point2_leaves:
        ...
        if nckh_pct > 0:
            ...
            if nckh_pct == 60.0:
                if r_start_full < pd.to_datetime(
                    tf_start
                ) or r_end_full > pd.to_datetime(tf_end):
                    nckh_pct = 30.0
    ...
    for seg in seg_data:
        ...
        for r, rule in point2_leaves:
            ...
            if inter_start <= inter_end:
                ...
                seg_override = (
                    (override_val * (seg_days / total_leaf_days))
                    if has_override
                    else None
                )
                seg_intervals.append(
                    (inter_start, inter_end, rule, r_end_full, seg_override)
                )
        ...
        for s, e, rule, r_end_full, seg_override in seg_intervals[1:]:
            if s <= cur_end:
                ...
                if rule["nckh_reduction_pct"] > cur_rule["nckh_reduction_pct"]:
                    cur_rule = rule
                    cur_override = (
                        seg_override if seg_override is not None else cur_override
                    )
            else:
                merged.append(
                    (cur_start, cur_end, cur_rule, cur_r_end_full, cur_override)
                )
                cur_start, cur_end, cur_rule, cur_r_end_full, cur_override = (
                    s,
                    e,
        ...
        for m_start, m_end, rule, m_r_end_full, m_override in merged:
            if m_override is not None:
                ...
                inter_weeks = min(
                    calculate_t04_weeks(m_start, m_r_end_full, holidays_list),
                    seg["weeks"],
                )
            else:
...
def _calculate_point3_reductions(
    ...
    for seg in seg_data:
        ...
        for r, rule in point3_leaves:
            ...
            if inter_start <= inter_end:
                ...
                seg_intervals.append(
                    (inter_start, inter_end, rule, r_end, seg_override)
                )
        ...
        for s, e, rule, r_end, seg_override in seg_intervals[1:]:
            if s <= cur_end:
                ...
                merged_rule["teaching_reduction_pct"] = max(
                    cur_rule["teaching_reduction_pct"], rule["teaching_reduction_pct"]
                )
                merged_rule["nckh_reduction_pct"] = max(
                    cur_rule["nckh_reduction_pct"], rule["nckh_reduction_pct"]
                )
                ...
                cur_start, cur_end, cur_rule, cur_r_end, cur_override = (
                    s,
                    e,
                    rule,
                    r_end,
        ...
        for m_start, m_end, rule, m_r_end, m_override in merged:
            ...
            while curr_date <= m_end:
                ...
                for p2_r, _ in point2_leaves:
                    p2_start = max(
                        pd.to_datetime(tf_start), pd.to_datetime(p2_r["start_date"])
                    )
                    p2_end = min(
                        pd.to_datetime(tf_end), pd.to_datetime(p2_r["end_date"])
                    )
            ...
            if working_days:
                ...
                if m_override is not None:
                    inter_weeks = m_override * (
                        len(working_days) / ((m_end - m_start).days + 1)
                    )
                else:
                    for b_start, b_end in blocks:
                        inter_weeks += calculate_t04_weeks(
                            b_start, b_end, holidays_list
                        )

                if seg["weeks"] > 0:
                    red_gc = (
                        seg["req_gc"]
                        * (rule["teaching_reduction_pct"] / 100.0)
                        * (inter_weeks / seg["weeks"])
                    )
                    red_nckh = (
                        seg["req_nckh"]
                        * (rule["nckh_reduction_pct"] / 100.0)
                        * (inter_weeks / seg["weeks"])
                    )
                else:
                    red_gc = 0.0
...
def _apply_auto_compensation(
    row,
    nckh_to_gc_ratio=3.0,
    gc_to_nckh_ratio=3.0,
    min_direct_teaching_ratio=0.5,
    min_nckh_ratio=0.25,
):
    ...
    # GC deficit, NCKH excess: compensate GC using excess NCKH (nckh_to_gc_ratio NCKH = 1 GC)
        ...
        min_required_teaching = (
            row["dinh_muc_gc_phai_thuc_hien"] * min_direct_teaching_ratio
        )
...
def calculate_teacher_metrics(
    teacher_id=None, timeframe_id=None, df_session_override=None
):
...
def _teacher_metrics_impl(teacher_id, timeframe_id, df_session_override):
    ...
    min_direct_teaching_ratio = float(
        get_setting_value("min_direct_teaching_ratio", "0.50")
    )
    ...
    df_hist = pd.read_sql_query(
        hist_query,
        conn,
        params=[tf_end.strftime("%Y-%m-%d"), tf_start.strftime("%Y-%m-%d")],
    )
    df_hist["start_date"] = pd.to_datetime(df_hist["start_date"])
    df_hist["end_date"] = pd.to_datetime(df_hist["end_date"]).fillna(
        pd.to_datetime(tf_end)
    )
    ...
    for _, teacher in df_teachers.iterrows():
        ...
        segments = _generate_timeline_segments(
            tf_start, tf_end, title_recs, dept_recs, role_recs, rules_dict
        )
        ...
        for seg_start, seg_end in segments:
            ...
            # Find active TITLE
            title_name = ""
            active_title = title_recs[
                (title_recs["start_date"] <= midpoint)
                & (title_recs["end_date"] >= midpoint)
            ]
            if not active_title.empty:
                ...
                if not sorted_titles.empty:
                    before_titles = sorted_titles[
                        sorted_titles["start_date"] <= midpoint
                    ]
            ...
            dept_name = ""
            active_dept = dept_recs[
                (dept_recs["start_date"] <= midpoint)
                & (dept_recs["end_date"] >= midpoint)
            ]
            ...
            if (
                ...
                and active_role.get("actual_weeks_override") != ""
            ):
                override = float(active_role["actual_weeks_override"])
                role_start = max(
                    pd.to_datetime(tf_start), pd.to_datetime(active_role["start_date"])
                )
                role_end = min(
                    pd.to_datetime(tf_end), pd.to_datetime(active_role["end_date"])
                )
            ...
            if (
                dept_name in depts_dict
                and depts_dict[dept_name]["is_teaching_dept"] == 0
            ):
                if role_t_red < 60.0:
                    role_t_red = 60.0
                    if (
                        "Tự động giảm 60% giờ chuẩn (Phòng/Trung tâm)"
                        not in applied_reductions
                    ):
            ...
            seg_yearly_nvk = (
                1760.0
                - (seg_base_gc * (1 - role_t_red / 100.0) * 3.0)
                - (seg_base_nckh * nckh_factor * (1 - role_n_red / 100.0))
            )
    ...
    if df_metrics.empty:
        df_metrics = pd.DataFrame(
            columns=[
                ...
                "dinh_muc_nckh_phai_thuc_hien",
            ]
        )

    df_out = pd.merge(df_teachers, df_metrics, on="id", how="left")
    df_out["dinh_muc_gc_phai_thuc_hien"] = df_out["dinh_muc_gc_phai_thuc_hien"].fillna(
        0
    )
    df_out["dinh_muc_nckh_phai_thuc_hien"] = df_out[
        "dinh_muc_nckh_phai_thuc_hien"
    ].fillna(0)
    ...
    if not df_session.empty:
        ...
        if not df_logs.empty:
            ...
            for tid, group in df_logs.groupby("teacher_id"):
                gc_dict[tid] = group[group["category"].isin(GC_CATEGORIES)][
                    "calculated_hours"
                ].sum()
                nckh_dict[tid] = group[group["category"].isin(NCKH_CATEGORIES)][
                    "calculated_hours"
                ].sum()
                hdcm_bd_dict[tid] = group[
                    group["category"].isin({"Hoạt động chuyên môn", "Bồi dưỡng"})
                ]["calculated_hours"].sum()
                nvk_dict[tid] = group[group["category"] == "Chấp hành Nhiệm vụ khác"][
                    "calculated_hours"
                ].sum()
                direct_teaching_dict[tid] = group[group["category"] == "Giảng dạy"][
                    "calculated_hours"
                ].sum()
    ...
    try:
        ...
        if not df_overrides.empty:
            for _, ovr in df_overrides.iterrows():
                ...
                if not idx.empty:
                    df_out.loc[idx, "tổng_gc_da_thuc_hien"] = ovr[
                        "tong_gc_da_thuc_hien"
                    ]
                    df_out.loc[idx, "nckh_da_thuc_hien"] = ovr["nckh_da_thuc_hien"]
                    df_out.loc[idx, "so_gio_duoc_mien_giam"] = ovr[
                        "so_gio_duoc_mien_giam"
                    ]
                    df_out.loc[idx, "dinh_muc_gc_phai_thuc_hien"] = ovr[
                        "dinh_muc_gc_phai_thuc_hien"
                    ]
    ...
    df_out["hoan_thanh_gd"] = df_out["gc_vuot_thieu"].apply(
        lambda x: "Đạt" if x >= 0 else "Không đạt"
    )
    df_out["hoan_thanh_nckh"] = df_out["nckh_vuot_thieu"].apply(
        lambda x: "Đạt" if x >= 0 else "Không đạt"
    )
    df_out["hoan_thanh_nvk"] = df_out["nvk_vuot_thieu"].apply(
        lambda x: "Đạt" if x >= 0 else "Không đạt"
    )

    def overall_status(row):
        if (
            row["hoan_thanh_gd"] == "Đạt"
            and row["hoan_thanh_nckh"] == "Đạt"
            and row["hoan_thanh_nvk"] == "Đạt"
        ):
            return "Đạt"
        return "Không đạt"
    ...
    if not df_out.empty:
        df_out["gc_sau_bu_tru"], df_out["nckh_sau_bu_tru"] = zip(
            *df_out.apply(
    ...
    df_conv = pd.read_sql_query(
        "SELECT * FROM manual_conversions WHERE timeframe_id = ?", conn, params=[tf_id]
    )
    ...
    df_out[["gc_vuot_thieu_sau_quy_doi", "nckh_vuot_thieu_sau_quy_doi"]] = df_out.apply(
        apply_conversions, axis=1
    )
...
def get_conversion_limits(teacher_id, timeframe_id, teacher_row=None):
    ...
    if row["nckh_vuot_thieu_sau_quy_doi"] > 0 and row["gc_vuot_thieu_sau_quy_doi"] < 0:
        ...
        if row["giang_day_truc_tiep"] >= min_required_teaching:
            ...
            res["warning"] = (
                f"Không đủ điều kiện: Số giờ giảng trực tiếp ({row['giang_day_truc_tiep']:.1f}) chưa đạt {min_direct_teaching_ratio * 100:.0f}% định mức ({min_required_teaching:.1f})."
            )
    ...
    if row["gc_vuot_thieu_sau_quy_doi"] > 0 and row["nckh_vuot_thieu_sau_quy_doi"] < 0:
        ...
        if nckh_done >= (nckh_norm * min_nckh_ratio):
            ...
            res["warning"] = (
                f"Không đủ điều kiện: Phải hoàn thành tối thiểu {min_nckh_ratio * 100:.0f}% định mức NCKH mới được quy đổi Giảng dạy bù sang NCKH."
            )
...
def calculate_department_compensation(df_teachers):
    ...
    for dept, group in df.groupby("dept_name"):
        ...
        for idx, row in excesses.iterrows():
            give_amount = transfer_amount * (
                row["gc_vuot_thieu_sau_quy_doi"] / total_excess
            )
            df.at[idx, "gc_vuot_thieu_sau_quy_doi"] -= give_amount
            df.at[idx, "gc_give_to_dept"] = give_amount

        for idx, row in deficits.iterrows():
            receive_amount = transfer_amount * (
                abs(row["gc_vuot_thieu_sau_quy_doi"]) / total_deficit
            )
...
def get_teacher_formula_breakdown(teacher_id, timeframe_id):
    ...
    # Timeframe name
    tf_row = pd.read_sql_query(
        "SELECT name FROM timeframes WHERE id = ?", conn, params=[tf_id]
    )
    tf_name = tf_row.iloc[0]["name"] if not tf_row.empty else str(tf_id)

    # Teacher
    t_row = pd.read_sql_query(
        "SELECT * FROM teachers WHERE id = ?", conn, params=[int(teacher_id)]
    )
    if t_row.empty:
        conn.close()
        return None
    ...
    for seg_start, seg_end in segments_raw:
        ...
        ad = dept_recs[
            (dept_recs["start_date"] <= midpoint) & (dept_recs["end_date"] >= midpoint)
        ]
        if not ad.empty:
            ...
            if not st_d.empty:
                ...
                dept_name = (
                    bd.iloc[-1]["value_text"]
                    if not bd.empty
                    else st_d.iloc[0]["value_text"]
                )
        ...
        for _, r in role_recs.iterrows():
            ...
            if rid in rules_dict and rules_dict[rid]["rule_type"] == "ROLE":
                if (
                    pd.to_datetime(r["start_date"])
                    <= midpoint
                    <= pd.to_datetime(r["end_date"])
                ):
                    active_role = r
                    rl = rules_dict[rid]
        ...
        if (
            ...
            wd = _count_workdays_detailed(
                seg_start, seg_end, holidays_list, holidays_named
            )
    ...
    for _, r in role_recs.iterrows():
        ...
        if has_override:
            ...
            wd_r = _count_workdays_detailed(
                r_start, r_end, holidays_list, holidays_named
            )
