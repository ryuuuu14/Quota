...
class FuzzyMatcher(MatcherStrategy):
    ...
    def match(
        ...
        candidates = [
            (h, norm_h)
            for norm_h, h in available_headers.items()
        ...
        if best_match:
            ...
            alternatives = [self._make_alternative(matched_header, score, match_type)]
        ...
        synonym_fuzzy = self._try_synonym_fuzzy(
            expected_column, candidates, used_headers
        )
        return synonym_fuzzy

    def _find_best(
        self, norm_expected: str, candidates: List[tuple]
    ) -> Optional[tuple]:
    ...
    def _difflib_match(
        ...
        if matches:
            ...
            score = int(
                difflib.SequenceMatcher(None, norm_expected, matches[0]).ratio() * 100
            )
            return (header, score, MatchType.FUZZY)
        return None

    def _try_synonym_fuzzy(
        self, expected_column: str, candidates: List[tuple], used_headers: Set[str]
    ) -> Optional[MatchCandidate]:
        ...
        for norm_syn, _ in synonyms:
            ...
            if best:
                ...
                return self._make_candidate(
                    ...
                    alternatives=[
                        self._make_alternative(
                            matched_header, score, MatchType.SYNONYM_FUZZY
                        )
                    ],
                )
        return None
