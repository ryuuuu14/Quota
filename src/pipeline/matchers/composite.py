...
class CompositeMatcher:
    ...
    def _build_chain(self) -> None:
        ...
        if matcher_configs.get("exact", {}).get("enabled", True):
            self.matchers.append(
                ExactMatcher(self.registry, matcher_configs.get("exact", {}))
            )
        if matcher_configs.get("synonym", {}).get("enabled", True):
            self.matchers.append(
                SynonymMatcher(self.registry, matcher_configs.get("synonym", {}))
            )
        if matcher_configs.get("fuzzy", {}).get("enabled", True):
            self.matchers.append(
                FuzzyMatcher(self.registry, matcher_configs.get("fuzzy", {}))
            )
    ...
    def match_all(
        ...
        for expected in expected_columns:
            ...
            candidate = self._match_single(
                expected, norm_expected, available_headers, used_headers
            )
        ...
        unmatched = self._find_unmatched(
            excel_headers, available_headers, used_headers, expected_columns
        )
        missing_required = [
            m.expected_column
            for m in mappings
            if m.is_required and m.matched_header is None
        ]
    ...
    def _detect_duplicates(
        ...
        for m in mappings:
            if m.matched_header:
                header_to_expected.setdefault(m.matched_header, []).append(
                    m.expected_column
                )
        ...
        for header, expected_list in header_to_expected.items():
            if len(expected_list) > 1:
                warnings.append(
                    DuplicateWarning(
                        excel_header=header, expected_columns=expected_list
                    )
                )
        return warnings
    ...
    def _suggest_for_unmatched(
        self, norm_header: str, expected_columns: List[str]
    ) -> List[Alternative]:
        suggestions = []
        for expected in expected_columns:
            norm_expected = self.registry.normalize(expected)
            ...
            try:
                ...
                score = int(
                    difflib.SequenceMatcher(None, norm_header, norm_expected).ratio()
                    * 100
                )
            ...
            if score >= 50:
                suggestions.append(
                    Alternative(
                        header=expected, confidence=score, match_type=MatchType.FUZZY
                    )
                )
