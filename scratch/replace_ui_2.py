...
with codecs.open('f:/annd/Quota/src/pages/4_CaiDatHeThong.py', 'r', 'utf-8') as f:
    lines = f.readlines()
...
for i, line in enumerate(lines):
    if "Ghi nhận các ngày nghỉ trong năm học (Tết Nguyên đán, Nghỉ hè, 30/4, nghỉ do bão lũ, v.v.)." in line:
        ...
        out_lines.append('<b>QUAN TRỌNG (Luật 44 tuần):</b> Luật bắt buộc 44 tuần làm việc + 8 tuần nghỉ (Tết + Hè).<br>\n')
        out_lines.append('Thiếu dữ liệu 8 tuần nghỉ → Hệ thống đếm lố tuần làm việc (>44) → Bật Cap (ép định mức bảo vệ) → Sai miễn giảm (177.2 → 186.7).<br>\n')
        out_lines.append('Bắt buộc: Khai báo đủ 8 tuần nghỉ Tết/Hè để hệ thống trừ lùi về 44 tuần chuẩn.\n')
        out_lines.append('</p>\n')
        ...
        out_lines.append('                    st.success("Đã nạp 8 tuần nghỉ. Cap đã tắt, định mức trở về chuẩn 44 tuần!")\n')
        out_lines.append('                    st.rerun()\n')
        out_lines.append('                else:\n')
        out_lines.append('                    st.error("Chưa có Năm học.")\n')
        ...
        if "Tính toán miễn giảm (như thai sản, đi học) sẽ bị sai lệch nếu chưa cấu hình đủ Nghỉ hè và Tết (cần tối thiểu ~40 ngày)" in line:
            line = line.replace("Tính toán miễn giảm (như thai sản, đi học) sẽ bị sai lệch nếu chưa cấu hình đủ Nghỉ hè và Tết (cần tối thiểu ~40 ngày)", "Tính toán miễn giảm lệch do Cap kích hoạt. Cần tối thiểu ~56 ngày (8 tuần)")
            line = line.replace("< 40:", "< 50:")
        
        out_lines.append(line)

with codecs.open('f:/annd/Quota/src/pages/4_CaiDatHeThong.py', 'w', 'utf-8') as f:
