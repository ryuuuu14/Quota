...
from auth import authenticate_user, get_scoped_teacher_ids
...
def setup_test_db(tmp_path):
    ...
    for dept_name, code in departments:
        cursor.execute(
            "INSERT OR IGNORE INTO departments (name, is_teaching_dept, dept_code) VALUES (?, 1, ?)",
            (dept_name, code),
        )
    ...
    for name, dept in teachers:
        cursor.execute(
            "INSERT INTO teachers (name, subject_group, employment_type) VALUES (?, 'Tổ bộ môn', 'TEACHER')",
            (name,),
        )
        teacher_id = cursor.lastrowid
        cursor.execute(
            "INSERT INTO teacher_role_history (teacher_id, record_type, value_text, start_date) VALUES (?, 'DEPARTMENT', ?, '2026-01-01')",
            (teacher_id, dept),
        )
...
def test_authenticate_user_plaintext_migration():
    ...
    cursor.execute(
        "SELECT password FROM admin_users WHERE username = ?", ("legacy_user",)
    )
...
def test_get_scoped_teacher_ids():
    ...
    dept_2_teachers = [
        r[0] for r in rows if r[1] == "Tự nhiên, Kỹ thuật, Ngoại ngữ, Tin học"
    ]
    ...
    head_user_1 = {
        "username": "head1",
        "role": "head_dept",
        "department_name": "Chính trị, Pháp luật, Nghiệp vụ",
    }
    ...
    head_user_2 = {
        "username": "head2",
        "role": "head_dept",
        "department_name": "Tự nhiên, Kỹ thuật, Ngoại ngữ, Tin học",
    }
