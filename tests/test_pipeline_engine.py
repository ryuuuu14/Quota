...
from pipeline import sanitize_value
from pipeline.validator import (
    validate_teachers_data,
...
def test_ghost_rows_and_null_validations(tmp_path):
    ...
    df = pd.DataFrame(
        [
            ...
            {"Đơn vị": None, "Mã GV": None, "Họ tên": None, "Chức vụ": None},
        ]
    )
    ...
    assert (
        "Ngày thực hiện 'invalid-date' không đúng định dạng YYYY-MM-DD." in row_1_errors
    )
...
def test_category_specific_activity_validation(tmp_path):
    ...
    cursor.execute(
        "INSERT OR IGNORE INTO teachers (id, name, subject_group, employment_type) VALUES (1, 'Teacher A', 'Bộ môn Toán', 'TEACHER')"
    )
    conn.commit()
