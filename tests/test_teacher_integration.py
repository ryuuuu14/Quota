...
def seed_activity_types(conn, cursor):
    ...
    for a in activities:
        try:
            cursor.execute(
                "INSERT INTO activity_types (name, category, unit, base_conversion_rate, is_teaching_activity, is_nckh_activity) VALUES (?,?,?,?,?,?)",
                a,
            )
        except Exception:
            pass
...
def seed_teacher_LeVanD(conn, cursor, tf_id):
    ...
    cursor.execute(
        "INSERT INTO teachers (name, subject_group, is_female) VALUES (?, ?, ?)",
    ...
    dt_rule_id = parse_reduction_rule_id(
        cursor, "Đi thực tế / Trưng tập (dưới 6 tháng)"
    )
    cursor.execute(
        """
        INSERT INTO teacher_role_history (teacher_id, record_type, value_text, reduction_rule_id, start_date, end_date)
    ...
    dh_rule_id = parse_reduction_rule_id(
        cursor, "Đi học / Bồi dưỡng (từ 6 đến dưới 10 tháng)"
    )
    cursor.execute(
        """
        INSERT INTO teacher_role_history (teacher_id, record_type, value_text, reduction_rule_id, start_date, end_date)
...
def seed_teacher_Simple(conn, cursor, tf_id):
    ...
    cursor.execute(
        "SELECT id FROM activity_types WHERE name = 'NCKH - Bài báo ISI/Scopus'"
    )
    nckh_id = cursor.fetchone()[0]
    cursor.execute(
        "SELECT id FROM activity_types WHERE name = 'NVK - Coi thi kết thúc HP'"
    )
...
def run_system_tests(conn, cursor, tf_id):
    ...
    for _, row in df.iterrows():
        print(f"    {row['name']}:")
        print(
            f"      title={row['title_name']}, base_gc={row['base_gc']}, base_nckh={row['base_nckh']}"
        )
        print(
            f"      dinh_muc_gc_phai_thuc_hien={row['dinh_muc_gc_phai_thuc_hien']:.2f}"
        )
    ...
    if not le_row.empty:
        ...
        print(
            "    Regulation: Định mức = [280×70%×17/44]+[280×60%×27/44] = 178,8  (ideal 44-week split)"
        )
        print(
            f"    System:     Định mức = {r['dinh_muc_gc_phai_thuc_hien']:.2f}  (exact working-day calculation)"
        )
        print("    Regulation: Giảm    = [75,7×8/17]+[103,1×3/27] = 47,1")
        print(f"    System:     Giảm    = {r['so_gio_duoc_mien_giam']:.2f}")
        print(
            f"    → diff: định mức={r['dinh_muc_gc_phai_thuc_hien'] - 178.8:.2f}  giảm={r['so_gio_duoc_mien_giam'] - 47.1:.2f}"
        )
        ...
        assert_approx(
            ...
            178.8,
            tolerance=5.4,
            label="Lê Văn D — định mức ≈ 178,8 (±3% working-day tolerance)",
        )
        assert_approx(
    ...
    if not bt_row.empty:
        ...
        print("    GV base=250 (Điều 6.2 social) → GVC base=280 (Điều 6.2 social)")
        print(
            f"    System: định mức={r['dinh_muc_gc_phai_thuc_hien']:.2f}, giảm={r['so_gio_duoc_mien_giam']:.2f}"
        )
        ...
        actual_ratio = (
            r["so_gio_duoc_mien_giam"] / r["dinh_muc_gc_phai_thuc_hien"]
            if r["dinh_muc_gc_phai_thuc_hien"] > 0
            else 0
        )
    ...
    if not gv_row.empty:
        ...
        expected_gc = (
            110.0  # 100 tiết lý thuyết (40HV, rate=1.0) + 10 buổi coi thi (HĐCM)
        )
        ...
        print(
            f"    GV social base GC = 250. System: {r['dinh_muc_gc_phai_thuc_hien']:.2f} (due to working-day weeks / 44)"
        )
        assert_approx(
            r["dinh_muc_gc_phai_thuc_hien"],
            250.0,
            tolerance=2.5,
            label="GV Bình Thường — định mức ≈ 250 (±2.5, working-day tolerance)",
        )

        # Cross-check: actual_weeks ≈ dinh_muc / base * 44
        implied_weeks = r["dinh_muc_gc_phai_thuc_hien"] / 250.0 * 44.0
        print(
            f"      Implied working weeks in TF: {implied_weeks:.2f} (should be ≈ 44)"
        )
    else:
